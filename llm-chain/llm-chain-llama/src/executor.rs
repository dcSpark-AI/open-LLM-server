...
use llm_chain_llama_sys::llama_get_embeddings;
use std::slice;
...
impl Executor {
    ...
    fn run_model(&self, input: LlamaInvocation) -> Output {
        let output = run_model(&self.context, input, self.context_params(), &self.callback);
        output
    }

    // Run the LLAMA model with the provided input to fill context, and then generate embeddings from it.
    pub fn generate_embeddings(&self, input_text: &str) -> Vec<i32> {
        let input = PerInvocation::new().to_invocation(input_text);
        // Running just to build context for embeddings
        let _ = run_model(&self.context, input, self.context_params(), &self.callback);
        let embed_pointer = unsafe { llama_get_embeddings(**&self.context) };
        let slice = unsafe { slice::from_raw_parts(embed_pointer, 1) };
        let result = slice.iter().map(|&f| f as i32).collect();
